...
public class FingerTargetNew : MonoBehaviour
{
    ...
    public GameObject fingertipDebugSphere;  // Debug sphere (auto-created if empty)

    [Header("Hit Detection")]
    public float hitDetectionRadius = 0.05f; // Distance required to "hit" the target (5cm default)

    [Header("Trajectory Logging")]
    public bool enableTrajectoryLogging = true;  // Toggle trajectory logging
    public float trajectoryLogRate = 30f;        // Hz - how often to log trajectory (30 times per second)
    ...
    {
        // Only log if trajectory logging is enabled and we're in an active movement phase
