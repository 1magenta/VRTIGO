...
public class SkewDisplayRot : MonoBehaviour
{
    ...
    public string phase = "Both"; // Start with Both active
    ...
    void OnEnable()
    {
        ...
        if (autoStartEnabled)
        {
            StartCoroutine(StartPhase(1f)); // Auto-start the phase sequence
        }
    }
    ...
    IEnumerator ShowCompletionIndicator()
    {
        ...
        LeftObject1.SetActive(false);
        RightObject2.SetActive(false);
        activeObject = "TestComplete";

        Debug.Log("Test of Skew completed - showing end indicator");

        // Hold completion state for 3 seconds to clearly indicate end
        yield return new WaitForSeconds(3f);

        // Signal final completion to task manager
        phase = "None";
        Debug.Log("Test sequence finished");
    }
